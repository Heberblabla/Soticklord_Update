...
import com.waos.soticklord.Batalla_oculta
import com.waos.soticklord.GlobalData
import com.waos.soticklord.Mapa
import com.waos.soticklord.R


class jugador1 : AppCompatActivity() {
    //atributos
    var indice_tropas = 0
    var indice_reyes = 0
    var indice_waos = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 3⃣Crea una solicitud de anuncio
    ...
    private fun mostrarImagen(direccion: Int) {
        ...
        if (indice_waos == 0) { //  Mostrar reyes
            val claves = GlobalData.Diccionario_Reyes_amigos.keys.toList()

            if (claves.isNotEmpty()) {
                indice_reyes = (indice_reyes + direccion + claves.size) % claves.size
                val claveActual = claves[indice_reyes]
                val seleccionada = GlobalData.Diccionario_Reyes_amigos[claveActual]
